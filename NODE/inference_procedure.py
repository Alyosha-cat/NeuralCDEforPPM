...
from NODE.inference_environment import BatchInference
...
def inference_loop(model, 
    ...
    with torch.no_grad():
        ...
        pad_mask_global = inference_dataset[num_categoricals_pref] # (num_prefs, window_size)
        ...
        for valbatch_num, vdata in tqdm(enumerate(inference_dataloader), desc="Validation batch calculation"):
                vinputs = vdata[:-num_target_tens-1][0]
                vpad = vdata[-num_target_tens]
                # Assign all input tensors to GPU
                vinputs = vinputs.to(device)
                vpad = vpad.to(device)
                voutputs = model(vinputs, vpad)
