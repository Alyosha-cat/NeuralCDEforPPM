...
class EncDecLSTM_no_context(nn.Module):
    ...
    def forward(self, 
        ...
        if self.training:
            ...
            cat_emb_suf = self.act_emb(inputs[3].to(dtype=int, device=inputs[3].device)) # (batch_size, window_size, self.activity_emb_size)
            ...
            for dec_step in range(0, window_size):
                ...
                act_inputs = act_inputs.to(dtype=torch.int, device=act_inputs.device)
