...
def inference_loop(model, 
    ...
    with torch.no_grad():
        ...
        for valbatch_num, vdata in tqdm(enumerate(inference_dataloader), desc="Validation batch calculation"):
                ...
                MAE_rrt_stand, MAE_rrt_sum = infer_env.compute_rrt_results()
        ...
        avg_MAE_rrt_stand = (torch.sum(MAE_rrt_stand_global) / MAE_rrt_stand_global.shape[0]).item()
    ...
    return_list += [percentage_correct, mean_absolute_length_diff, mean_too_early, mean_too_late, avg_MAE_rrt_sum_minutes, avg_MAE_rrt_stand]
