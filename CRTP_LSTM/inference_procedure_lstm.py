...
def inference_loop(model, 
    ...
    with torch.no_grad():
        ...
        MAE_ttne_standard_global = torch.tensor(data=[], dtype=torch.float32).to(device)
        ...
        for valbatch_num, vdata in tqdm(enumerate(inference_dataloader), desc="Validation batch calculation"):
                ...
                MAE_ttne_seconds, MAE_ttne_standard = infer_env.compute_ttne_results() # shape (torch.sum(self.actual_length+1), )
                MAE_ttne_standard_global = torch.cat(tensors=(MAE_ttne_standard_global, MAE_ttne_standard), dim=-1) 
        ...
        avg_MAE_stand_ttne = (torch.sum(MAE_ttne_standard_global) / MAE_ttne_standard_global.shape[0]).item()
    ...
    return_list += [avg_MAE_stand_RRT, avg_MAE_minutes_RRT, val_loss_avg, avg_MAE_ttne_minutes, avg_MAE_stand_ttne]
